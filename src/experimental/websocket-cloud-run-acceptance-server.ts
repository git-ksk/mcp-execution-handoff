  ...
  if (url.pathname === "/acceptance-result") {
    sendJson(res, 200, {
      ...
      wssSurfaceFailureInputBoundaryStage:
        surface?.diagnosticsSnapshot().failureInputBoundaryStage ?? "none",
      ...
      wssSurfaceFailureHelperCrashOrigin:
        surface?.diagnosticsSnapshot().failureHelperCrashOrigin ?? "none",
      wssSurfaceLastHelperCrashErrorKind:
        surface?.diagnosticsSnapshot().lastHelperCrashErrorKind ?? "none",
      wssSurfaceFailureHelperCrashErrorKind:
        surface?.diagnosticsSnapshot().failureHelperCrashErrorKind ?? "none",
      wssSurfaceLastHelperCrashMessageClass:
        surface?.diagnosticsSnapshot().lastHelperCrashMessageClass ?? "none",
      wssSurfaceFailureHelperCrashMessageClass:
        surface?.diagnosticsSnapshot().failureHelperCrashMessageClass ?? "none",
