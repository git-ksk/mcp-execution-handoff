...
export interface HandoffRequestState { version: 1; phase: "awaiting_human"; toolName: string; argsDigest: string; interventionId: string; epoch: number; resumeStrategy: HandoffResumeStrategy; principalBinding: string; }
...
export function createHandoffRequestState(input: { toolName: string; args: unknown; interventionId: string; epoch: number; resumeStrategy: HandoffResumeStrategy; principalBinding: string; }): HandoffRequestState {
  return { version: 1, phase: "awaiting_human", toolName: input.toolName, argsDigest: digestToolInvocation(input.toolName, input.args), interventionId: input.interventionId, epoch: input.epoch, resumeStrategy: input.resumeStrategy, principalBinding: input.principalBinding };
}
export function handoffStateMatchesInvocation(state: HandoffRequestState, toolName: string, args: unknown, expectedPrincipalBinding: string): boolean {
  return state.version === 1 && state.phase === "awaiting_human" && state.toolName === toolName && state.argsDigest === digestToolInvocation(toolName, args) && state.principalBinding === expectedPrincipalBinding;
}
