...
function finishGesture(g,x,y){if(!g.moved&&Math.hypot(x-g.startX,y-g.startY)>18)g.moved=true;if(g.moved){sendGestureScroll(g,x,y);return}if(g.editable||keyboardMode){focusKeyboard()}else{keyboard.blur()}const sentAt=performance.now();if(sendCritical({kind:'tap',x:g.point.x,y:g.point.y}))lastTapSentAt=sentAt}
