...
function closePeer(){clearRelayTimer();clearMetricsTimer();lastTapSentAt=0;editableRegions=[];editableRegionsAt=0;setKeyboardControlsVisible(false);armKeyboardFallback();video.style.opacity='0';if(pc){try{pc.ontrack=null;pc.onconnectionstatechange=null;pc.close()}catch{}pc=null}critical=null;realtime=null}
...
keyboardOpen.addEventListener('click',function(){if(stopped)return;focusKeyboard()});keyboardBackspace.addEventListener('click',function(event){event.preventDefault();if(stopped)return;if(sendCritical({kind:'key',key:'Backspace'}))focusKeyboard()});
