...
function verifyingWebRtcPageHtml(nonce: string): string {
...
function completedWebRtcPageHtml(nonce: string): string {
...
export class TakeoverBroker {
  ...
  private readonly webRtcTerminalVerificationSessions = new Set<string>();
  private readonly verifyingWebRtcSessions = new Set<string>();
  ...
  createNativeLink(
    ...
    if (target) {
      ...
      if (target.windowId !== undefined) this.nativeTargetWindowIds.set(locator.id, target.windowId);
    }
  ...
  createWebRtcLink(
    ...
    target?: TakeoverHostTarget,
    inputPolicy?: WebRtcHumanInputPolicy,
    options?: { terminalTargetBehavior?: "verifying" }
  ): string | undefined {
    ...
    if (options?.terminalTargetBehavior !== undefined && options.terminalTargetBehavior !== "verifying") return undefined;
    ...
    if (this.verifyingWebRtcSessions.has(locator.id)) return undefined;
    if (this.webRtcOnlySessions.has(locator.id)
      && this.webRtcTerminalVerificationSessions.has(locator.id) !== terminalVerification) return undefined;
    ...
    if (terminalVerification) this.webRtcTerminalVerificationSessions.add(locator.id);
    else this.webRtcTerminalVerificationSessions.delete(locator.id);
    if (target) {
  ...
  async completeWebRtcAfterVerification(intervention: TakeoverInterventionRef): Promise<boolean> {
  ...
  async handle(
    ...
    if (operation === "webrtc-connect") {
      ...
      try {
        ...
        const diagnosticStages = this.webRtcRuntime.diagnosticsSnapshot().events
          ...
          .join(",");
        console.error(
          `[mcp-execution-handoff] WebRTC connect failed code=${runtimeCode}${startStage ? ` stage=${startStage}` : ""}${startReason ? ` reason=${startReason}` : ""}${startSignalingState ? ` signaling_state=${startSignalingState}` : ""}${startEndCause ? ` end_cause=${startEndCause}` : ""} diagnostics=${diagnosticStages || "none"}`
        );
        try {
          this.sessions.releaseClientGeneration(id, boundPrincipal, clientBinding, binding.clientGeneration);
        } catch {}
    ...
    if (operation === "webrtc-suspend") {
      ...
      try {
        if (this.webRtcRuntime.suspend) await this.webRtcRuntime.suspend(id);
        else await this.webRtcRuntime.revoke(id);
      } catch {
  ...
  private webRtcHooks(binding: WebRtcTakeoverRuntimeBinding) {
    return {
      ...
      terminal: (cause: WebRtcRuntimeEndCause) => {
        if (!this.webRtcTerminalVerificationSessions.has(binding.takeoverSessionId)
          || !webRtcTerminalCauseRequiresVerification(cause)) return false;
...
function webRtcTerminalCauseRequiresVerification(cause: WebRtcRuntimeEndCause): boolean {
...
function inputAllowed(policy: WebRtcHumanInputPolicy, kind: WebRtcHumanInput["kind"]): boolean {
  if (kind === "pointer_button") return policy.tap;
  return policy[kind];
}
