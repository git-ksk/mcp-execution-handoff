...
function webRtcClientScript(): string {
  return `(() => {
const parts=location.pathname.split('/').filter(Boolean);const sessionId=parts.length?parts[parts.length-1]:'';
const statusEl=document.querySelector('#status');const video=document.querySelector('#video');const screen=document.querySelector('.screen');const zoomButton=document.querySelector('#zoom');const keyboard=document.querySelector('#keyboard');const keyboardOpen=document.querySelector('#keyboard-open');const keyboardBackspace=document.querySelector('#keyboard-backspace');const doneButton=document.querySelector('#done');let completionCapability=doneButton&&doneButton.dataset?doneButton.dataset.completion||'':'';
let clientBinding=randomClientBinding();let cap='';let reconnectHandle='';let clientGeneration=0;let inputPolicy={tap:true,scroll:true,text:true,key:true};let pc=null;let critical=null;let realtime=null;let stopped=false;let suspended=false;let suspendPromise=Promise.resolve();let gesture=null;let pinch=null;let viewScale=1;let viewPanX=0;let viewPanY=0;let orientationResetTimer=0;let composing=false;let keyboardMode=false;let failureInProgress=false;let initialReconnectUsed=false;let connectionStartedAt=0;let firstFrameMs=null;let metricsSamplesSent=0;let inputMetricsSamplesSent=0;let metricsTimer=0;let latestFrameMetrics={};let lastTapSentAt=0;let relayState='disabled';let relayTimer=0;let editableRegions=[];let editableRegionsAt=0;let keyboardFallbackTimer=0;let iceCandidateCounts={host:0,srflx:0,prflx:0,relay:0};let iceGatherStartedAt=0;
const MARK='_';
...
function resetViewTransform(){pinch=null;gesture=null;applyViewTransform(1,0,0)}
