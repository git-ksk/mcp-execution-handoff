...
function createManagedHandoff(): BrowserHandoffAdapter {
  return new BrowserHandoffAdapter({
    ...
    ...(wssOnlyConfigured ? { transportPolicy: { order: ["websocket_relay"] as const } } : {}),
    onComplete: () => {
      completionCount += 1;
      doneObserved = true;
  ...
  if (teardownCompleted && observedWebSocketPath && !staleWebSocketLocatorRejected) {
    ...
    if (sessionId) {
      const stale = await handoff.handle(
        new Request(new URL(`/takeover/api/websocket-bootstrap/${sessionId}`, publicBaseUrl), { method: "POST" }),
        principal
      );
