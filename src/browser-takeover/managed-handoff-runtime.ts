...
interface ManagedHandoffSession {
  ...
  completed: boolean;
  readonly diagnosticEvents: ManagedOperatorDiagnosticEvents;
...
export class ManagedWindowHandoffRuntime {
  ...
  constructor(config: ManagedWindowHandoffRuntimeConfig) {
    ...
    if (needsWebSocket && config.successorWindowPolicy && webSocketPlatform !== "macos") {
      throw new WindowHandoffCoreError(
        ...
        "successor-window lineage over managed WSS requires the macOS exact-window backend"
      );
    }
    if (needsWebSocket && config.successorWindowPolicy
      ...
      throw new WindowHandoffCoreError(
        "SUCCESSOR_POLICY_INVALID",
        "macOS WSS successor lineage requires an explicit WSS-only transport plan"
      );
    }
  ...
  managedOperatorDiagnosticsSnapshot(
    ...
    const degraded = !failed && wssCurrent && (
      disconnectKind !== "none"
      || surfaceFailure !== "none"
  ...
  operatorDiagnosticsSnapshot(source: Extract<OperatorDiagnosticsSource, "browser_handoff" | "window_handoff">): OperatorDiagnosticsSnapshot {
    ...
    const snapshot = session?.coordinator.diagnosticsSnapshot() ?? {
      ...
      previousTransport: "none" as const,
      generation: 0,
      transitionCount: 0,
  ...
  async #patchWebRtcClient(response: Response): Promise<Response> {
    ...
    script = script.replace(
      initialMarker,
      "resetKeyboardSession();resetViewTransform();armKeyboardFallback();armManagedReadyTimeout();void connect('claim').catch(async function(){closePeer();if(await managedTransportFallback())return;if(relayState==='unavailable')status('Secure relay unavailable');else status('Session unavailable or connection failed');stopped=true});"
    );
  ...
  async #patchWebSocketPage(response: Response, session: ManagedHandoffSession): Promise<Response> {
    ...
    html = html.replace(
      disconnectHookMarker,
      "function onWebSocketDisconnected(ws,event){if(stopped||terminalPending)return;resetViewTransform();void managedWebSocketDisconnected(ws,event)}"
    );
    html = html.replace(
      initialFailureHookMarker,
      "function onInitialWebSocketConnectFailure(){if(!stopped&&!terminalPending)void managedTransportFallback()}"
    );
    ...
    html = html.replace(
      initialMarker,
      "controls();resetViewTransform();window.addEventListener('orientationchange',scheduleOrientationReset);armManagedReadyTimeout();void connect().catch(()=>onInitialWebSocketConnectFailure())"
    );
