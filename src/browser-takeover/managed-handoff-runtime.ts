...
export class ManagedWindowHandoffRuntime {
  ...
  async #patchWebSocketPage(response: Response, session: ManagedHandoffSession): Promise<Response> {
    ...
    html = html.replace(
      ...
      "function onWebSocketDisconnected(ws,event){if(stopped||terminalPending)return;void managedWebSocketDisconnected(ws,event)}"
    );
    html = html.replace(
    ...
    html = html.replace(
      initialMarker,
      "controls();armManagedReadyTimeout();void connect().catch(()=>onInitialWebSocketConnectFailure())"
    );
