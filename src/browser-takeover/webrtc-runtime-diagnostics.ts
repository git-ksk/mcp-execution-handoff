...
function withHostExitDiagnostics(spawnProcess: typeof spawn): typeof spawn {
...
function observeHostExit(child: ChildProcess): void {
  const stderr = child.stderr;
  let reason: WebRtcHostExitReason | undefined;
  let buffer = "";
  ...
  if (stderr) {
    stderr.on("data", (chunk: Buffer | string) => {
      ...
      for (;;) {
        ...
        if (!line.startsWith(HOST_EXIT_PREFIX)) continue;
        const value = line.slice(HOST_EXIT_PREFIX.length);
        if (HOST_EXIT_REASON.test(value)) reason = value as WebRtcHostExitReason;
      }
    });
  ...
  child.once("close", (code, signal) => {
    ...
    if (!reason && signal === "SIGTERM") return;
    const exitCode = boundedExitCode(code);
    const exitSignal = boundedExitSignal(signal);
    console.error(
      `[mcp-execution-handoff] WebRTC host exited reason=${reason ?? "unexpected"} exit_code=${exitCode} signal=${exitSignal}`
    );
  });
}

function boundedExitCode(value: number | null): string {
  if (value === null || !Number.isSafeInteger(value) || value < 0 || value > 255) return "none";
  return String(value);
}

function boundedExitSignal(value: NodeJS.Signals | null): string {
