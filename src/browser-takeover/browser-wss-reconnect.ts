...
export function browserWssCloseIsReconnectable(code: number): boolean {
  return code === 1001 || code === 1006;
}

/** Short bounded exponential retry: 250, 500, 1000, 2000, 2000 ms. */
export function browserWssReconnectDelayMs(attempt: number): number {
  if (!Number.isSafeInteger(attempt) || attempt < 0) return 2_000;
  return Math.min(2_000, 250 * (2 ** Math.min(attempt, 3)));
}

/** Emit the same pure reconnect helpers into the isolated browser page. */
export function browserWssReconnectClientSource(): string {
  return [
