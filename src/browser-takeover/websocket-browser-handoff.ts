...
import {
  ...
  type ExperimentalWebSocketWindowHandoffConfig,
...
export class ExperimentalWebSocketBrowserHandoff {
  readonly #window: ExperimentalWebSocketWindowHandoff;
  readonly #publicOrigin: string;
  readonly #sessionsById = new Map<string, BrowserSession>();
  readonly #sessionsByIntervention = new Map<string, BrowserSession>();

  constructor(config: ExperimentalWebSocketBrowserHandoffConfig) {
    if (!config.takeover.publicBaseUrl) throw new Error("Browser WSS Handoff requires a public base URL");
  ...
  start(request: ExperimentalWebSocketBrowserStartRequest): string {
    ...
    if (existing && existing.sessionId !== sessionId) this.#forget(existing);
  ...
  async handle(request: Request, boundPrincipal: string | undefined): Promise<Response> {
    ...
    if (page && this.#sessionsById.has(page[1]!)) {
      ...
      headers.set(
        ...
        browserContentSecurityPolicy(nonce, this.#publicOrigin)
      );
      return new Response(
        request.method === "HEAD" ? null : browserPageHtml(nonce, state.inputPolicy),
        { status: 200, headers }
      );
    }
  ...
  #forget(state: BrowserSession): void {
...
function browserResponse(status: number, contentType: string, body: string): Response {
