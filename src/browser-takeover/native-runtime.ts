 ...
 */
export class InheritedFdNativeRuntimeProvider implements NativeTakeoverRuntimeProvider {
  private readonly active = new Map<string, ActiveNativeRuntime>();
  private readonly spawnProcess: typeof spawn;
  private readonly inputPort: number;
  private readonly controlPort: number;
  private readonly videoFeedbackPort: number;

  constructor(private readonly config: InheritedFdNativeRuntimeProviderConfig) {
    ...
    if (isIP(controlBindHost) === 0) throw new Error("native runtime control bind host must be an IP literal");
    ...
    for (const port of [this.inputPort, this.controlPort, this.videoFeedbackPort]) {
      if (!validPort(port)) throw new Error("native runtime ports must be 1-65535");
    }
  ...
  async begin(
    ...
    if (existing) {
      if (existing.binding.clientGeneration === binding.clientGeneration) {
        throw new NativeTakeoverRuntimeError(
          "NATIVE_BOOTSTRAP_ALREADY_ISSUED",
          "native bootstrap for this client generation has already been issued"
        );
      }
    ...
    try {
      ...
      throw new NativeTakeoverRuntimeError(
        "NATIVE_RUNTIME_START_FAILED",
        `native runtime failed to start: ${error instanceof Error ? error.message : "unknown error"}`
      );
    }

    const active: ActiveNativeRuntime = {
      binding: { ...binding },
      rootKey,
      sessionHashHex,
      child
  ...
  async revoke(takeoverSessionId: string): Promise<void> {
    ...
    try {
      ...
      throw new NativeTakeoverRuntimeError(
        "NATIVE_RUNTIME_REVOKE_FAILED",
        `native runtime revoke failed: ${error instanceof Error ? error.message : "unknown error"}`
      );
    } finally {
  ...
  private async sendAuthenticatedRevoke(active: ActiveNativeRuntime): Promise<void> {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      THIN_TAKEOVER_SESSION_KEY_FD: "3",
    ...
    const child = this.spawnProcess(
      this.config.revokeExecutable,
      [this.config.controlBindHost ?? "127.0.0.1", String(this.controlPort)],
