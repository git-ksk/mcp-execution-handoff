...
- Screen Recording and Accessibility permission preflight before the Human-control session starts;
...
- Linux editable-region/focus hints come from a read-only native AT-SPI helper bound to the exact X11 target process/window: only process ancestry, role/state, and component extents are queried, traversal is bounded, no accessible text/name/value is requested, and unavailable/ambiguous metadata degrades to empty/non-editable hints; cloud-like Chromium targets must expose the native bridge at process launch (`ACCESSIBILITY_ENABLED=1` with `--force-renderer-accessibility=form-controls` in the acceptance fixture);
- the Browser compatibility profile stays capped at 1280×720 / 3 Mbps / 30 fps and uses Constrained Baseline H.264 (`42c01f`); first-class macOS Window Handoff opts into `window_text`, which never upscales the source and raises only the ceiling to 1920×1080 / 5 Mbps / 30 fps with encoding-speed priority disabled; encoder admission remains one in-flight frame plus one latest pending encoded frame for both profiles;
...
The same target includes a harmless macOS pointer-activation matrix for ordinary `<button>`, checkbox default activation, focus-only input, a legacy `javascript:` anchor, and an ordinary navigation link followed by a second ordinary link. The target records only boolean acceptance state; no entered text or remote input payload is retained. Start a fresh locator through `/__new`, complete the pointer matrix on the Human-control session, then read `http://127.0.0.1:8891/__state` locally and require `pointerComplete: true`. This makes the browser-pointer regression repeatable without any Maps/Cinema/provider selector, DOM `.click()` fallback, or target-service state.
...
When a target process is bound, the macOS WebRTC host resolves the unique AX window whose frame matches the captured `inputBounds`, uses `AXRaise` only as a best-effort activation aid, and admits Human input only after the same process is active and the focused AX window still matches the exact captured bounds. Some first-party system windows reject `AXRaise` even while already focused; that rejection is therefore not itself an authority failure. Ambiguity, target disappearance, ownership mismatch, or final focused-window mismatch still fails closed. This keeps window-scoped capture and Human input on the same target even when another Mac application was frontmost before the remote action.

A dedicated physical secure-system-UI harness is available with `npm run accept:window:macos-secure-ui`. It binds only the exact System Settings process/window resolved at session start, enables pointer tap only, refuses TURN credentials so the run is unambiguously LAN-direct, and exposes diagnostics only on loopback. The accepted action is limited to the Accessibility pane **Add (+)** control; the operator must not select a file or change a permission. On macOS 26.5, physical iPhone acceptance opened the system file chooser. The original captured Accessibility window became greyed while the separate chooser was focused, demonstrating that capture did not silently widen to the new window or desktop. No TCC value, credential, password, or authorization decision is part of this acceptance.

Physical iPhone Safari media-quality acceptance for #56 also passed on both path classes without changing authority or backpressure semantics. The same bounded Window media policy used 5 Mbps with encoding-speed priority disabled. A same-LAN direct run rendered a native System Settings source without upscaling and was judged visibly sharper; identifier-free diagnostics recorded direct RTT p50 40 ms and host encode p50 4.9 ms. A cellular relay run selected `Live · relay`, exposed relay candidates on both peers, rendered 1200×916 at 5 Mbps, and recorded relay RTT p50 61 ms, host encode p50 7.7 ms, RTP drain p50 0.9 ms, and Human input acknowledgement p50 354 ms. No frame contents, candidate strings, addresses, credentials, or input payloads were retained.
