...
Werift never uses its implicit third-party STUN default. Without an explicit provider, the browser stays host-only and the server uses only the reviewed explicit STUN boundary. With either TURN adapter, the broker first binds the exact intervention / epoch / principal / client-generation / expiry tuple, then the Handoff runtime creates separate short-lived ICE credentials for browser and server peers. Normal ICE policy remains `all`, not `relay`, so direct candidate pairs remain preferred/eligible and TURN is a reachability fallback. Reconnect creates a new generation and new ICE session; suspend, peer disconnect, Done, Cancel, pagehide/background, intervention revoke, and expiry tear down the peer and revoke Handoff media/input authority. Cloudflare short-lived allocations are actively revoked. coturn TURN REST has no per-credential revoke API, so its randomized relay credentials remain usable only until the expiry timestamp derived from the same generation deadline; possession of that residual relay credential does not restore Handoff signaling, DataChannel authority, or target-service authentication. A stale capability, reconnect handle, or generation cannot reattach to the prepared or active ICE session.
...
Self-hosted coturn is a deployment-controlled relay trust boundary. `MCP_HANDOFF_COTURN_SHARED_SECRET` is long-lived server-side secret material and MUST receive the same handling as the Cloudflare API token. Handoff never places principal, intervention, client, account, or target-service identifiers in coturn REST usernames; it uses random peer identifiers so the relay sees only the expiry timestamp plus unlinkable random material. `MCP_HANDOFF_COTURN_TURN_URLS` accepts only `turn:`/`turns:` endpoints without embedded credentials; optional `MCP_HANDOFF_COTURN_STUN_URLS` accepts only `stun:`/`stuns:` endpoints. Configuring more than one TURN provider family or only part of one family fails closed at runtime construction.
