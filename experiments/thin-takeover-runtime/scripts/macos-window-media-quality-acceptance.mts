...
function assertSingleSystemSettingsWindow(): void {
  ...
  if (count !== 1) throw new Error("Media-quality acceptance requires exactly one initial System Settings window");
}

const LAN_HOST = process.env.HANDOFF_LAN_HOST || defaultLanHost();
const PORT = Number(process.env.HANDOFF_WINDOW_MEDIA_PORT || "8896");
...
const PRINCIPAL = "macos-window-media-quality-physical-acceptance";
...
if (process.env.MCP_HANDOFF_CLOUDFLARE_TURN_KEY_ID || process.env.MCP_HANDOFF_CLOUDFLARE_TURN_KEY_API_TOKEN) {
  throw new Error("Refusing direct media-quality LAN acceptance while TURN credentials are present");
}
...
const interventionId = `window-media-${randomBytes(6).toString("hex")}`;
const locator = handoff.start({
  intervention: { id: interventionId, epoch: 1 },
  principalBinding: PRINCIPAL,
  target: { processId: TARGET_PID },
  inputPolicy: { tap: false, scroll: false, text: false, key: false }
});
...
console.log("Window media-quality physical acceptance ready (direct-only, view-only)");
console.log(`Locator: ${locator}`);
console.log(`Local diagnostics: http://127.0.0.1:${PORT}/__diag`);
console.log("Expected action: open the locator on iPhone and visually inspect native System Settings text/UI. No remote input is enabled for this run.");
