...
function assertSingleSystemSettingsWindow(): void {
  ...
  if (count !== 1) throw new Error("Window-lineage acceptance requires exactly one initial System Settings window");
}

const LAN_HOST = process.env.HANDOFF_LAN_HOST || defaultLanHost();
const PORT = Number(process.env.HANDOFF_WINDOW_LINEAGE_PORT || "8895");
...
const PRINCIPAL = "macos-window-lineage-physical-acceptance";
...
if (process.env.MCP_HANDOFF_CLOUDFLARE_TURN_KEY_ID || process.env.MCP_HANDOFF_CLOUDFLARE_TURN_KEY_API_TOKEN) {
  throw new Error("Refusing Window-lineage LAN acceptance while TURN credentials are present");
}
...
const interventionId = `window-lineage-${randomBytes(6).toString("hex")}`;
...
console.log("Window-lineage physical acceptance ready");
console.log(`Locator: ${locator}`);
console.log(`Local diagnostics: http://127.0.0.1:${PORT}/__diag`);
console.log("Expected action: on iPhone, use Aim to tap Accessibility Add (+) once. The same session should rotate to the new file chooser. Do not select a file or change a permission.");
