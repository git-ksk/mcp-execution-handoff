...
- The HTTPS/WSS managed-runtime evaluation from #40 has been promoted through #155/#156 into the Handoff-owned Browser/Window fallback sequence `WebRTC direct -> WebSocket relay -> optional WebRTC/TURN relay`. Deterministic and Cloud Run-equivalent container acceptance are green; #152 remains open for the production-shaped `run.app` physical iPhone acceptance on the exact candidate revision.
...
- **WebSocket** — supported second-stage HTTPS/WSS managed-runtime fallback after direct WebRTC. It can avoid TURN entirely for the WSS leg while reusing the exact-window host helpers, one-client lease, generation fencing, revoke semantics, and bounded latest-frame policy.
...
The supported managed order is now `WebRTC direct -> WebSocket relay -> optional WebRTC/TURN relay`. Browser/Window consumers keep one Handoff lifecycle abstraction and do not select WebSocket/ICE/TURN providers. Stale direct-WebRTC and WSS generations fail closed, no admitted input is replayed across transports, disconnect still does not mean Done, and exact process/window plus Human input policy remain unchanged across fallback. The deterministic suite, repeated Linux normal-browser WebRTC acceptance, and Cloud Run-equivalent bounded WSS container acceptance are green on the #157 candidate. Parent #152 intentionally remains open until the exact current candidate also passes production-shaped `run.app` physical iPhone Safari acceptance, including induced direct-WebRTC unavailability, selected `websocket_relay`, stale pre-fallback generation/revoke checks, and teardown. Numeric same-session latency comparison against direct WebRTC/TURN remains unrecorded; see `experiments/websocket-cloud-run/COMPARISON.md`, which makes no unsupported numeric performance claim.
