...
class TerminalDataChannelRuntime {
    ...
    async start(binding, offer, hooks) {
        ...
        if (existing)
            await this.revokeActive(binding.takeoverSessionId);
    ...
    sendOutput(takeoverSessionId, generation, dataBase64) {
        const runtime = this.requireGeneration(takeoverSessionId, generation);
    ...
    sendState(runtime, state) {
        if (!runtime.channel || runtime.channel.readyState !== "open")
            return;
        const payload = JSON.stringify({ kind: "state", state });
        if (runtime.channel.bufferedAmount <= 64 * 1024)
            runtime.channel.send(payload);
    }
    requireGeneration(takeoverSessionId, generation) {
        const runtime = this.#active.get(takeoverSessionId);
    ...
    async revokeActive(takeoverSessionId) {
        const runtime = this.#active.get(takeoverSessionId);
        if (!runtime)
            return;
...
export class ExperimentalTerminalWebRtcTakeover {
    ...
    start(interventionId, epoch, principalBinding) {
        ...
        if (this.#session && !this.#session.completed)
            throw new Error("terminal WebRTC takeover already active");
    ...
    activateHuman(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
    ...
    fenceHuman(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
     ...
     */
    noteTransportDisconnect(interventionId, epoch, generation) {
        const session = this.requireIntervention(interventionId, epoch);
        if (!Number.isSafeInteger(generation) || generation < 1 || session.completed || session.clientGeneration !== generation)
            return;
        session.humanActive = false;
        session.disconnected = true;
    }
    drainEvents(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
    ...
    nextEvent(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
    ...
    pushOutput(interventionId, epoch, dataBase64) {
        const session = this.requireIntervention(interventionId, epoch);
        if (!session.humanActive || session.clientGeneration === undefined)
            throw new Error("terminal Human output unavailable");
    ...
    releaseCompleted(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
    ...
    async revoke(interventionId, epoch) {
        const session = this.requireIntervention(interventionId, epoch);
        session.humanActive = false;
    ...
    async handle(request) {
        ...
        if (page)
            return this.handlePage(request, page[1]);
        ...
        if (operation === "prepare")
            return this.handlePrepare(request, session, clientBinding);
        return this.handleConnect(request, session, clientBinding);
    }
    handlePage(request, id) {
        if (request.method !== "GET" && request.method !== "HEAD")
            return json(405, { error: "method_not_allowed" });
        const session = this.#session;
        if (!session || id !== session.id || session.completed)
            return json(404, { error: "takeover_unavailable" });
        try {
    ...
    async handlePrepare(request, session, clientBinding) {
    ...
    async handleConnect(request, session, clientBinding) {
    ...
    acceptMessage(session, generation, message) {
        if (this.#session !== session || session.completed || session.disconnected || session.faulted || !session.humanActive)
            return;
        if (session.clientGeneration !== generation)
            return;
        let value;
        try {
    ...
    requireIntervention(interventionId, epoch) {
        const session = this.#session;
...
function terminalEventCost(event) {
...
function parseTerminalEvent(value) {
    ...
    if (record.kind === "done" && Object.keys(record).length === 1)
        return { kind: "done" };
    return undefined;
}
function terminalPage(nonce) {
    return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>Terminal takeover</title><style nonce="${nonce}">:root{font-family:system-ui,-apple-system,sans-serif;color-scheme:dark}body{margin:0;background:#080a0d;color:#f5f5f5}main{max-width:900px;margin:auto;padding:14px}pre{min-height:52vh;max-height:62vh;overflow:auto;white-space:pre-wrap;word-break:break-word;background:#020304;border:1px solid #333;border-radius:12px;padding:12px;font:14px ui-monospace,SFMono-Regular,Menlo,monospace}.row{display:flex;gap:8px}input{flex:1;min-width:0}input,button{font:inherit;min-height:44px;border-radius:9px;border:1px solid #555;padding:8px;background:#171a1f;color:#fff}button:disabled,input:disabled{opacity:.45}.done{width:100%;margin-top:10px}small{display:block;opacity:.72;line-height:1.4;margin-top:10px}</style></head><body><main><h3>Terminal takeover</h3><div id="status" role="status" aria-live="polite">Connecting…</div><pre id="terminal"></pre><div class="row"><input id="line" disabled autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" maxlength="1024" placeholder="Type harmless terminal input"><button id="send" disabled>Send</button></div><button id="done" class="done" disabled>Done</button><small>This controls only the current bounded PTY. Do not enter passwords, tokens, 2FA codes, or other secrets during this acceptance. Terminal escape sequences are rendered as plain text and are not executed by this page.</small><script nonce="${nonce}">(()=>{const parts=location.pathname.split('/').filter(Boolean);const id=parts[parts.length-1]||'';const status=document.querySelector('#status');const term=document.querySelector('#terminal');const field=document.querySelector('#line');const sendButton=document.querySelector('#send');const doneButton=document.querySelector('#done');const decoder=new TextDecoder();let pc=null,channel=null,cap='',human=false,stopped=false,uiState='connecting';function client(){const b=new Uint8Array(16);crypto.getRandomValues(b);return Array.from(b,x=>x.toString(16).padStart(2,'0')).join('')}const clientBinding=client();function b64(bytes){let s='';for(let i=0;i<bytes.length;i++)s+=String.fromCharCode(bytes[i]);return btoa(s)}function unb64(s){const raw=atob(s);const b=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)b[i]=raw.charCodeAt(i);return b}function originHeaders(){return {'x-terminal-client':clientBinding}}async function api(op,options){const o=options||{};const h=Object.assign({},o.headers||{},originHeaders());const r=await fetch('/takeover/terminal/api/'+op+'/'+encodeURIComponent(id),Object.assign({cache:'no-store'},o,{headers:h}));if(!r.ok)throw new Error('unavailable');return r}function waitIce(peer){if(peer.iceGatheringState==='complete')return Promise.resolve();return new Promise(resolve=>{const finish=()=>{if(peer.iceGatheringState==='complete'){peer.removeEventListener('icegatheringstatechange',finish);resolve()}};peer.addEventListener('icegatheringstatechange',finish);setTimeout(()=>{peer.removeEventListener('icegatheringstatechange',finish);resolve()},10000)})}const uiLabels={connecting:'Connecting…',connected:'Connected · waiting for Human authority',human_active:'Human authority active',verifying:'Done · verifying',connection_unavailable:'Connection unavailable',session_unavailable:'Session unavailable'};function setUiState(next,force){if(uiState==='verifying'&&!force&&next!=='verifying')return;uiState=next;const active=next==='human_active';human=active;field.disabled=!active;sendButton.disabled=!active;doneButton.disabled=!active;status.textContent=uiLabels[next]||uiLabels.session_unavailable}function message(event){try{const m=JSON.parse(String(event.data));if(m.kind==='state'){if(m.state==='connected')setUiState('connected');else if(m.state==='human_active')setUiState('human_active');else if(m.state==='fenced'&&uiState!=='verifying')setUiState('connected');return}if(m.kind==='output'&&typeof m.dataBase64==='string'){term.textContent+=decoder.decode(unb64(m.dataBase64));if(term.textContent.length>65536)term.textContent=term.textContent.slice(-65536);term.scrollTop=term.scrollHeight}}catch{}}async function connect(){const prep=await (await api('prepare',{method:'POST'})).json();cap=prep.capability;pc=new RTCPeerConnection({iceServers:prep.webrtcIce.iceServers,iceTransportPolicy:'all'});channel=pc.createDataChannel('terminal-control',{ordered:true});channel.onopen=()=>{setUiState('connected')};channel.onmessage=message;channel.onclose=()=>{if(!stopped)setUiState('connection_unavailable')};pc.onconnectionstatechange=()=>{if(!stopped&&(pc.connectionState==='failed'||pc.connectionState==='disconnected'||pc.connectionState==='closed'))setUiState('connection_unavailable')};const offer=await pc.createOffer();const ice=waitIce(pc);await pc.setLocalDescription(offer);await ice;const r=await api('connect',{method:'POST',headers:{'content-type':'application/json','x-terminal-capability':cap},body:JSON.stringify({type:'offer',sdp:pc.localDescription.sdp})});const answer=await r.json();await pc.setRemoteDescription(answer.webrtc)}async function send(){if(!human||!channel||channel.readyState!=='open')return;const line=field.value;if(!line)return;const bytes=new TextEncoder().encode(line+'\\n');if(bytes.length>2048)return;field.value='';channel.send(JSON.stringify({kind:'input',dataBase64:b64(bytes)}))}sendButton.onclick=()=>void send();field.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();void send()}});doneButton.onclick=()=>{if(uiState!=='human_active'||!channel||channel.readyState!=='open')return;setUiState('verifying');stopped=true;try{channel.send(JSON.stringify({kind:'done'}));setTimeout(()=>{try{channel&&channel.close();pc&&pc.close()}catch{}},150)}catch{setUiState('session_unavailable',true)}};setUiState('connecting');void connect().catch(()=>{setUiState('session_unavailable',true);stopped=true})})();</script></main></body></html>`;
}
function validIntervention(id, epoch) {
    return /^[A-Za-z0-9_-]{1,128}$/.test(id) && Number.isSafeInteger(epoch) && epoch > 0;
}
function validBinding(value) {
    return /^[a-f0-9]{64}$/.test(value);
}
function readClientBinding(value) {
...
//# sourceMappingURL=terminal-webrtc.js.map