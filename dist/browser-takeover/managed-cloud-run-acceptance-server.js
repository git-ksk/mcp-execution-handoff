...
let browserTarget;
let targetInitStage = "not_started";
let targetInitFailure;
...
let activePrincipal;
let activeLocator;
let initialDirectPath;
let observedWebSocketPath;
...
server.on("upgrade", (req, socket, head) => {
    if (!handoff || !handoff.handleUpgrade(req, socket, head))
        socket.destroy();
});
server.listen(port, "0.0.0.0");
await once(server, "listening");
void initializeBrowser().catch((error) => {
...
async function handleHttp(req, res) {
    ...
    if (url.pathname === "/acceptance-status") {
        ...
        if (!principal || principal !== activePrincipal)
            return sendJson(res, 404, { error: "not_found" });
        await refreshManagedEvidence();
    ...
    if (url.pathname === "/start") {
        ...
        if (!target)
            return sendJson(res, 503, { error: "target_not_ready" });
        ...
        await refreshManagedEvidence();
        res.setHeader("set-cookie", `${COOKIE_NAME}=${cookieValue}; Secure; HttpOnly; SameSite=Strict; Path=/; Max-Age=900`);
        if (!hadCookie) {
    ...
    if (!handoff || !handoff.ownsPath(url.pathname))
        return sendJson(res, 404, { error: "not_found" });
    const principal = principalFromRequest(req);
...
async function refreshManagedEvidence() {
    if (!handoff)
        return;
    const snapshot = handoff.operatorDiagnosticsSnapshot();
    if (snapshot.source !== "browser_handoff")
        return;
    const transport = snapshot.transport;
    if (transport.namespace !== "managed_handoff")
        return;
    managedEvidence = {
...
async function probeStaleLocators() {
    const principal = activePrincipal;
    if (!handoff || !principal)
        return;
    if (!staleDirectLocatorRejected
...
function acceptanceSnapshot() {
    ...
    return {
        ...
        wssSurfaceInputBoundaryStage: handoff?.managedSurfaceDiagnosticsSnapshot().lastInputBoundaryStage ?? "none",
        ...
        wssSurfaceFailureInputStage: handoff?.managedSurfaceDiagnosticsSnapshot().failureInputStage ?? "none",
        wssSurfaceFailureInputBoundaryStage: handoff?.managedSurfaceDiagnosticsSnapshot().failureInputBoundaryStage ?? "none",
        wssSurfaceLastInputFailureDetail: handoff?.managedSurfaceDiagnosticsSnapshot().lastInputFailureDetail ?? "none",
        wssSurfaceFailureInputFailureDetail: handoff?.managedSurfaceDiagnosticsSnapshot().failureInputFailureDetail ?? "none",
        wssSurfaceLastHelperStopReason: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperStopReason ?? "none",
        wssSurfaceFailureHelperStopReason: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperStopReason ?? "none",
        wssSurfaceLastHelperCrashReason: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperCrashReason ?? "none",
        wssSurfaceFailureHelperCrashReason: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperCrashReason ?? "none",
        wssSurfaceLastHelperExitKind: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperExitKind ?? "none",
        wssSurfaceFailureHelperExitKind: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperExitKind ?? "none",
        wssSurfaceLastHelperCrashClass: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperCrashClass ?? "none",
        wssSurfaceFailureHelperCrashClass: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperCrashClass ?? "none",
        wssSurfaceLastHelperCrashOrigin: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperCrashOrigin ?? "none",
        wssSurfaceFailureHelperCrashOrigin: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperCrashOrigin ?? "none",
        wssSurfaceLastHelperCrashErrorKind: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperCrashErrorKind ?? "none",
        wssSurfaceFailureHelperCrashErrorKind: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperCrashErrorKind ?? "none",
        wssSurfaceLastHelperCrashMessageClass: handoff?.managedSurfaceDiagnosticsSnapshot().lastHelperCrashMessageClass ?? "none",
        wssSurfaceFailureHelperCrashMessageClass: handoff?.managedSurfaceDiagnosticsSnapshot().failureHelperCrashMessageClass ?? "none",
        ...
        wssFailureDisconnectKind: handoff?.managedWebSocketDiagnosticsSnapshot().failureDisconnectKind ?? "none",
        ...
        staleDirectGenerationFenced: staleDirectLocatorRejected && managedEvidence.generation >= 2 && managedEvidence.transitionCount >= 1,
...
async function initializeBrowser() {
    if (process.platform !== "linux")
        throw new Error("Cloud Run managed acceptance requires Linux");
    ...
    await waitFor("xvfb", async () => runQuietBounded("/usr/bin/xdpyinfo", ["-display", DISPLAY], xEnv)
        .then(() => true, () => false), 8_000);
    ...
    if (openbox.exitCode !== null)
        throw new Error("openbox unavailable");
    ...
    await waitFor("exact-window", async () => {
        ...
        if (owner === pid)
            exactWindowOwnerObserved = true;
        if (title === TARGET_WINDOW_TITLE)
            exactWindowTitleObserved = true;
...
function classifyTargetInitFailure(error) {
    const message = error instanceof Error ? error.message : "";
    if (message.includes("xvfb"))
        return "xvfb_unavailable";
    if (message.includes("openbox"))
        return "openbox_unavailable";
    if (message.includes("browser pid") || message.includes("browser exited") || message.includes("browser executable")) {
        return "browser_unavailable";
    }
    if (message.includes("target-page"))
        return "target_page_unavailable";
    if (message.includes("exact-window"))
        return "exact_window_unavailable";
    return "unknown";
}
function startBootstrapPage() {
...
async function readBoundedBody(req) {
    ...
    for await (const chunk of req) {
        ...
        if (bytes > MAX_HTTP_BODY_BYTES)
            throw new Error("Managed acceptance request body exceeds bound");
        chunks.push(buffer);
    }
...
function cookieValueFromRequest(req) {
    ...
    for (const part of cookie.split(";")) {
        ...
        if (name === COOKIE_NAME && value && /^[A-Za-z0-9_-]{32}$/.test(value))
            return value;
    }
    return undefined;
}
...
function requiredHttpsOrigin(value) {
    if (!value)
        throw new Error("HANDOFF_WSS_PUBLIC_BASE_URL is required");
...
function requiredGitRevision(value) {
    if (!value || !/^[0-9a-f]{40}$/.test(value)) {
        throw new Error("HANDOFF_ACCEPTANCE_REVISION must be the exact 40-character git SHA");
    }
    return value;
}
function assertNoTurnEnvironment() {
    for (const name of TURN_ENV) {
        if (process.env[name])
            throw new Error("Managed Cloud Run acceptance forbids TURN configuration");
    }
}
...
async function runQuietBounded(executable, args, env) {
    const child = spawn(executable, args, { env, stdio: "ignore" });
    child.once("error", () => undefined);
    const result = await Promise.race([
        once(child, "close")
            .then(([code]) => ({ kind: "closed", code }))
            .catch(() => ({ kind: "failed" })),
        new Promise((resolve) => {
...
async function runBounded(executable, args, env) {
    ...
    child.once("error", () => undefined);
    const result = await Promise.race([
        once(child, "close").then(([code]) => ({ kind: "closed", code })),
        new Promise((resolve) => {
            const timer = setTimeout(() => resolve({ kind: "timeout" }), COMMAND_TIMEOUT_MS);
            timer.unref();
        })
    ...
    if (result.code !== 0 || bytes > 64 * 1024)
        throw new Error("bounded acceptance command failed");
    return Buffer.concat(chunks).toString("utf8");
}
...
//# sourceMappingURL=managed-cloud-run-acceptance-server.js.map