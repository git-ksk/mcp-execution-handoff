...
import { browserOperatorStatusClientSource, browserOperatorStatusCss, browserOperatorStatusMarkup } from "./operator-status-surface.js";
...
let clientBinding=randomClientBinding();let cap='';let reconnectHandle='';let clientGeneration=0;let inputPolicy={tap:true,scroll:true,text:true,key:true};let pc=null;let critical=null;let realtime=null;let primaryButtonDown=false;let primaryButtonPoint=null;let primaryReleaseTimer=0;let stopped=false;let suspended=false;let suspendPromise=Promise.resolve();let generationReleasePromise=null;let reconnectPromise=null;let recoveryReconnectUsed=false;let gesture=null;let pinch=null;let viewScale=1;let viewPanX=0;let viewPanY=0;let aimMode=false;let orientationResetTimer=0;let compositionPhase='idle';let pendingKeyTimer=0;let keyboardMode=false;let keyboardMirror='';let failureInProgress=false;let connectionStartedAt=0;let firstFrameMs=null;let firstFrameTimer=0;let metricsSamplesSent=0;let inputMetricsSamplesSent=0;let metricsTimer=0;let latestFrameMetrics={};let lastTapSentAt=0;let relayState='disabled';let relayTimer=0;let editableRegions=[];let editableRegionsAt=0;let keyboardFallbackTimer=0;let iceCandidateCounts={host:0,srflx:0,prflx:0,relay:0};let iceGatherStartedAt=0;let completionState='idle';let verificationPollTimer=0;
...
function status(text){if(completionState!=='idle')return;statusEl.textContent=text;setBrowserOperatorStatus(text)}
function completionStatus(text){statusEl.textContent=text;setBrowserOperatorStatus(text)}
...
function clearPendingKeyboardKey(){if(pendingKeyTimer){clearTimeout(pendingKeyTimer);pendingKeyTimer=0}}
function resetKeyboardSession(){clearPendingKeyboardKey();compositionPhase='idle';keyboard.value='';keyboardMirror=''}
function focusKeyboard(){if(document.activeElement===keyboard)return;try{keyboard.focus({preventScroll:true})}catch{try{keyboard.focus()}catch{}}}
function sendKeyboardDelta(current,inputType,inputData){const delta=browserTextReplacementMutation(keyboardMirror,current,inputType,inputData);keyboardMirror=delta.next;if(delta.next!==current){keyboard.value=delta.next;try{keyboard.setSelectionRange(delta.next.length,delta.next.length)}catch{}}for(let i=0;i<delta.backspaces;i+=1){if(inputPolicy.key)sendCritical({kind:'key',key:'Backspace'})}let insert=delta.insert;if((inputType==='insertLineBreak'||inputType==='insertParagraph')&&insert){insert=insert.replace(/[\\r\\n]+$/,'')}if(insert&&inputPolicy.text)sendCritical({kind:'text',text:insert});if((inputType==='insertLineBreak'||inputType==='insertParagraph')&&inputPolicy.key)sendCritical({kind:'key',key:'Enter'})}
function scheduleKeyboardKey(key){if(!inputPolicy.key)return;clearPendingKeyboardKey();pendingKeyTimer=setTimeout(function(){pendingKeyTimer=0;if(compositionPhase==='idle')sendCritical({kind:'key',key:key})},250)}
function setKeyboardMode(enabled){const opening=enabled&&!keyboardMode;keyboardMode=enabled;keyboardOpen.setAttribute('aria-pressed',enabled?'true':'false');if(enabled){if(opening)resetKeyboardSession();focusKeyboard()}else keyboard.blur()}
...
keyboard.addEventListener('compositionstart',function(){clearPendingKeyboardKey();compositionPhase=browserImeNextCompositionPhase(compositionPhase,'composition_start')});keyboard.addEventListener('compositionend',function(){clearPendingKeyboardKey();compositionPhase='idle'});keyboard.addEventListener('keydown',function(event){if(browserImeKeyboardEventIsCompositionControlled(compositionPhase,event.isComposing,Number(event.keyCode)||0))return;if(event.key==='Backspace'||event.key==='Enter')scheduleKeyboardKey(event.key)});keyboard.addEventListener('input',function(event){const inputType=typeof event.inputType==='string'?event.inputType:'';clearPendingKeyboardKey();if(inputType==='insertCompositionText'||inputType==='deleteCompositionText')return;if(inputType==='insertFromComposition'){sendKeyboardDelta(keyboard.value,inputType,null);return}if(compositionPhase!=='idle'||event.isComposing)return;sendKeyboardDelta(keyboard.value,inputType,typeof event.data==='string'?event.data:null)});
...
resetKeyboardSession();resetViewTransform();armKeyboardFallback();void connect('claim').catch(function(){closePeer();if(relayState==='unavailable')status('Secure relay unavailable');else status('Session unavailable or connection failed');stopped=true});
})();`;
}
