...
function sendKeyboardDelta(current,inputType){const delta=browserTextReplacementDelta(keyboardMirror,current);keyboardMirror=current;for(let i=0;i<delta.backspaces;i+=1){if(inputPolicy.key)sendCritical({kind:'key',key:'Backspace'})}let insert=delta.insert;if((inputType==='insertLineBreak'||inputType==='insertParagraph')&&insert){insert=insert.replace(/[\\r\\n]+$/,'')}if(insert&&inputPolicy.text)sendCritical({kind:'text',text:insert});if((inputType==='insertLineBreak'||inputType==='insertParagraph')&&inputPolicy.key)sendCritical({kind:'key',key:'Enter'})}
