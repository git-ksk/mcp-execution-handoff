...
let clientBinding=randomClientBinding();let cap='';let reconnectHandle='';let clientGeneration=0;let inputPolicy={tap:true,scroll:true,text:true,key:true};let pc=null;let critical=null;let realtime=null;let primaryButtonDown=false;let primaryButtonPoint=null;let primaryReleaseTimer=0;let stopped=false;let suspended=false;let suspendPromise=Promise.resolve();let generationReleasePromise=null;let reconnectPromise=null;let recoveryReconnectUsed=false;let gesture=null;let pinch=null;let viewScale=1;let viewPanX=0;let viewPanY=0;let aimMode=false;let orientationResetTimer=0;let compositionPhase='idle';let compositionSettleTimer=0;let keyboardMode=false;let keyboardMirror='';let failureInProgress=false;let connectionStartedAt=0;let firstFrameMs=null;let firstFrameTimer=0;let metricsSamplesSent=0;let inputMetricsSamplesSent=0;let metricsTimer=0;let latestFrameMetrics={};let lastTapSentAt=0;let relayState='disabled';let relayTimer=0;let editableRegions=[];let editableRegionsAt=0;let keyboardFallbackTimer=0;let iceCandidateCounts={host:0,srflx:0,prflx:0,relay:0};let iceGatherStartedAt=0;let completionState='idle';let verificationPollTimer=0;
...
function clearCompositionSettle(){if(compositionSettleTimer){clearTimeout(compositionSettleTimer);compositionSettleTimer=0}}
function resetKeyboardBuffer(){clearCompositionSettle();compositionPhase='idle';keyboard.value='';keyboardMirror=''}
function finishKeyboardComposition(){if(compositionPhase!=='settling')return;clearCompositionSettle();compositionPhase=browserImeNextCompositionPhase(compositionPhase,'settled')}
function settleKeyboardComposition(){clearCompositionSettle();compositionSettleTimer=setTimeout(function(){compositionSettleTimer=0;if(compositionPhase!=='settling')return;compositionPhase=browserImeNextCompositionPhase(compositionPhase,'settled');syncKeyboardValue()},0)}
function focusKeyboard(){if(document.activeElement===keyboard)return;try{keyboard.focus({preventScroll:true})}catch{try{keyboard.focus()}catch{}}resetKeyboardBuffer()}function syncKeyboardValue(){if(compositionPhase!=='idle')return true;const delta=browserTextReplacementDelta(keyboardMirror,keyboard.value),remove=delta.backspaces,insert=delta.insert;if(remove>0&&!inputPolicy.key){status('Keyboard replacement unavailable');return false}for(let i=0;i<remove;i+=1){if(!sendCritical({kind:'key',key:'Backspace'}))return false}if(insert&&(!inputPolicy.text||!sendCritical({kind:'text',text:insert})))return false;keyboardMirror=keyboard.value;return true}function setKeyboardMode(enabled){keyboardMode=enabled;keyboardOpen.setAttribute('aria-pressed',enabled?'true':'false');if(enabled)focusKeyboard();else keyboard.blur()}
...
keyboard.addEventListener('compositionstart',function(){clearCompositionSettle();compositionPhase=browserImeNextCompositionPhase(compositionPhase,'composition_start')});keyboard.addEventListener('compositionend',function(){compositionPhase=browserImeNextCompositionPhase(compositionPhase,'composition_end');settleKeyboardComposition()});keyboard.addEventListener('keydown',function(event){if(browserImeKeyboardEventIsCompositionControlled(compositionPhase,event.isComposing,Number(event.keyCode)||0))return;if(event.key==='Backspace'&&inputPolicy.key&&keyboard.value===''){if(sendCritical({kind:'key',key:'Backspace'}))event.preventDefault();return}if(event.key==='Enter'&&inputPolicy.key){if(syncKeyboardValue()&&sendCritical({kind:'key',key:'Enter'})){event.preventDefault();resetKeyboardBuffer()}}});keyboard.addEventListener('beforeinput',function(event){if(browserImeInputEventIsCompositionControlled(compositionPhase,event.isComposing))return;if(event.inputType==='deleteContentBackward'&&inputPolicy.key&&keyboard.value===''){if(sendCritical({kind:'key',key:'Backspace'}))event.preventDefault();return}if((event.inputType==='insertLineBreak'||event.inputType==='insertParagraph')&&inputPolicy.key){if(syncKeyboardValue()&&sendCritical({kind:'key',key:'Enter'})){event.preventDefault();resetKeyboardBuffer()}}});keyboard.addEventListener('input',function(event){if(compositionPhase==='composing'||event.isComposing)return;if(compositionPhase==='settling')finishKeyboardComposition();const value=keyboard.value;if(inputPolicy.key&&(value.endsWith('\\n')||value.endsWith('\\r'))){keyboard.value=value.slice(0,-1);if(syncKeyboardValue()&&sendCritical({kind:'key',key:'Enter'}))resetKeyboardBuffer();return}syncKeyboardValue()});
