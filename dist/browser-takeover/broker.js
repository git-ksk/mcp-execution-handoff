        ...
        if (operation === "webrtc-connect") {
            ...
            try {
                ...
                const diagnosticStages = this.webRtcRuntime.diagnosticsSnapshot().events
                    ...
                    .join(",");
                console.error(`[mcp-execution-handoff] WebRTC connect failed code=${runtimeCode}${startStage ? ` stage=${startStage}` : ""}${startReason ? ` reason=${startReason}` : ""}${startSignalingState ? ` signaling_state=${startSignalingState}` : ""} diagnostics=${diagnosticStages || "none"}`);
                try {
                    this.sessions.releaseClientGeneration(id, boundPrincipal, clientBinding, binding.clientGeneration);
                }
                catch { }
