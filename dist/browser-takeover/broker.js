...
let clientBinding=randomClientBinding();let cap='';let reconnectHandle='';let clientGeneration=0;let inputPolicy={tap:true,scroll:true,text:true,key:true};let pc=null;let critical=null;let realtime=null;let primaryButtonDown=false;let primaryButtonPoint=null;let primaryReleaseTimer=0;let stopped=false;let suspended=false;let suspendPromise=Promise.resolve();let generationReleasePromise=null;let reconnectPromise=null;let recoveryReconnectUsed=false;let gesture=null;let pinch=null;let viewScale=1;let viewPanX=0;let viewPanY=0;let aimMode=false;let orientationResetTimer=0;let compositionPhase='idle';let suppressKeyboardInput=false;let keyboardInputReleaseTimer=0;let pendingKeyTimer=0;let keyboardMode=false;let failureInProgress=false;let connectionStartedAt=0;let firstFrameMs=null;let firstFrameTimer=0;let metricsSamplesSent=0;let inputMetricsSamplesSent=0;let metricsTimer=0;let latestFrameMetrics={};let lastTapSentAt=0;let relayState='disabled';let relayTimer=0;let editableRegions=[];let editableRegionsAt=0;let keyboardFallbackTimer=0;let iceCandidateCounts={host:0,srflx:0,prflx:0,relay:0};let iceGatherStartedAt=0;let completionState='idle';let verificationPollTimer=0;
...
function clearVerificationPoll(){if(verificationPollTimer){clearTimeout(verificationPollTimer);verificationPollTimer=0}}
function terminalCompleted(){if(completionState==='closed')return;clearVerificationPoll();completionState='closed';stopped=true;suspended=false;failureInProgress=false;completionCapability='';if(doneButton.dataset)doneButton.dataset.completion='';doneButton.disabled=true;doneButton.setAttribute('aria-disabled','true');doneButton.textContent='Closed';keyboard.blur();closePeer();try{if(typeof video.pause==='function')video.pause();video.srcObject=null}catch{}cap='';reconnectHandle='';clientGeneration=0;completionStatus('Remote control closed. Return to the requesting workflow.')}
async function readTerminalState(){try{const response=await fetch('/takeover/api/webrtc-state/'+encodeURIComponent(sessionId),{cache:'no-store'});if(!response.ok)return 'unavailable';const body=await response.json();return body&&body.state==='closed'?'closed':body&&body.state==='verifying'?'verifying':'unavailable'}catch{return 'unavailable'}}
function pollVerification(){clearVerificationPoll();if(completionState!=='verifying')return;verificationPollTimer=setTimeout(async function(){verificationPollTimer=0;const state=await readTerminalState();if(state==='closed'){terminalCompleted();return}if(state==='verifying'){pollVerification();return}completionStatus('Verification unavailable. Remote control remains closed.')},500)}
function verificationPending(){if(completionState==='closed'||completionState==='verifying')return;completionState='verifying';stopped=true;suspended=false;failureInProgress=false;completionCapability='';if(doneButton.dataset)doneButton.dataset.completion='';doneButton.disabled=true;doneButton.setAttribute('aria-disabled','true');doneButton.textContent='Verifying…';keyboard.blur();closePeer();try{if(typeof video.pause==='function')video.pause();video.srcObject=null}catch{}cap='';reconnectHandle='';clientGeneration=0;completionStatus('Verifying… Remote input is disabled.');pollVerification()}
...
function clearKeyboardTimers(){if(keyboardInputReleaseTimer){clearTimeout(keyboardInputReleaseTimer);keyboardInputReleaseTimer=0}if(pendingKeyTimer){clearTimeout(pendingKeyTimer);pendingKeyTimer=0}}
function resetKeyboardBuffer(){clearKeyboardTimers();compositionPhase='idle';suppressKeyboardInput=false;keyboard.value=''}
function suppressTrailingKeyboardInput(){suppressKeyboardInput=true;if(keyboardInputReleaseTimer)clearTimeout(keyboardInputReleaseTimer);keyboardInputReleaseTimer=setTimeout(function(){keyboardInputReleaseTimer=0;suppressKeyboardInput=false;keyboard.value=''},0)}
function commitKeyboardText(text){if(typeof text!=='string'||!text||!inputPolicy.text)return false;const sent=sendCritical({kind:'text',text:text});if(sent){keyboard.value='';suppressTrailingKeyboardInput()}return sent}
function settleKeyboardComposition(data){compositionPhase=browserImeNextCompositionPhase(compositionPhase,'composition_end');suppressKeyboardInput=true;if(keyboardInputReleaseTimer)clearTimeout(keyboardInputReleaseTimer);keyboardInputReleaseTimer=setTimeout(function(){keyboardInputReleaseTimer=0;if(compositionPhase!=='settling')return;const committed=typeof data==='string'&&data?data:keyboard.value;compositionPhase=browserImeNextCompositionPhase(compositionPhase,'settled');suppressKeyboardInput=false;if(committed)commitKeyboardText(committed);else keyboard.value=''},0)}
function scheduleKeyboardKey(key){if(!inputPolicy.key)return;if(pendingKeyTimer)clearTimeout(pendingKeyTimer);pendingKeyTimer=setTimeout(function(){pendingKeyTimer=0;if(compositionPhase==='idle')sendCritical({kind:'key',key:key})},0)}
function consumeScheduledKeyboardKey(){if(pendingKeyTimer){clearTimeout(pendingKeyTimer);pendingKeyTimer=0}}
function focusKeyboard(){if(document.activeElement===keyboard)return;try{keyboard.focus({preventScroll:true})}catch{try{keyboard.focus()}catch{}}resetKeyboardBuffer()}function setKeyboardMode(enabled){keyboardMode=enabled;keyboardOpen.setAttribute('aria-pressed',enabled?'true':'false');if(enabled)focusKeyboard();else keyboard.blur()}
...
function clearFirstFrameTimer(){if(firstFrameTimer){clearTimeout(firstFrameTimer);firstFrameTimer=0}}
function closePeer(){releasePrimaryButton();clearRelayTimer();clearMetricsTimer();clearFirstFrameTimer();lastTapSentAt=0;editableRegions=[];editableRegionsAt=0;keyboardMode=false;keyboardOpen.setAttribute('aria-pressed','false');keyboard.blur();setKeyboardControlsVisible(false);armKeyboardFallback();resetViewTransform();video.style.opacity='0';if(pc){try{pc.ontrack=null;pc.onconnectionstatechange=null;pc.close()}catch{}pc=null}critical=null;realtime=null}
...
async function suspend(){if(stopped||!cap)return;if(suspended){await suspendPromise;return}await releaseGeneration('Suspended · stale session closed')}
function reconnect(){if(stopped||!suspended||!reconnectHandle)return Promise.resolve();if(reconnectPromise)return reconnectPromise;const pending=(async()=>{await suspendPromise.catch(()=>{});if(stopped||!suspended||!reconnectHandle)return;clientBinding=randomClientBinding();const deadline=performance.now()+8000;for(let attempt=0;attempt<8&&performance.now()<deadline;attempt+=1){try{await connect('reconnect');failureInProgress=false;return}catch(e){closePeer();if(e&&e.status===409){status('Reconnecting · waiting for stale connection to close…');const hinted=Number(e.retryAfterMs);const fallback=Math.min(1000,250*(attempt+1));const remaining=Math.max(100,deadline-performance.now());await wait(Math.min(Number.isFinite(hinted)?hinted:fallback,remaining));continue}throw e}}throw new Error('reconnect unavailable')})();reconnectPromise=pending;pending.then(()=>{if(reconnectPromise===pending)reconnectPromise=null},()=>{if(reconnectPromise===pending)reconnectPromise=null});return pending}
function sendGestureScroll(g,x,y){const dx=x-g.lastX,dy=y-g.lastY;g.lastX=x;g.lastY=y;if(Math.hypot(x-g.startX,y-g.startY)>18)g.moved=true;if(g.moved&&(dx||dy))sendRealtime({kind:'scroll',deltaX:browserWebRtcScrollDelta(dx),deltaY:browserWebRtcScrollDelta(dy)})}
function finishGesture(g,x,y){if(!g.moved&&Math.hypot(x-g.startX,y-g.startY)>18)g.moved=true;if(g.moved){if(g.localPan)return;sendGestureScroll(g,x,y);return}if(aimMode)return;if(keyboardMode)focusKeyboard();const sentAt=performance.now();if(sendPrimaryTap(g.point))lastTapSentAt=sentAt}
function tapAimTarget(){if(!aimMode||stopped||!inputPolicy.tap)return;const r=screen.getBoundingClientRect();const p=mapPoint({clientX:r.left+r.width/2,clientY:r.top+r.height/2});if(!p){status('Aim target outside remote view');return}if(keyboardMode)focusKeyboard();const sentAt=performance.now();if(sendPrimaryTap(p))lastTapSentAt=sentAt}
function touchWithId(list,id){for(let i=0;i<list.length;i+=1)if(list[i].identifier===id)return list[i];return null}
video.addEventListener('touchstart',function(event){if(!touchEventsAvailable||stopped||!critical||critical.readyState!=='open')return;if(event.touches.length===2){beginPinch(event.touches);event.preventDefault();return}if(event.touches.length!==1||pinch)return;const touch=event.touches[0];const p=mapPoint(touch);if(!p)return;gesture={id:touch.identifier,pointerType:'touch',startX:touch.clientX,startY:touch.clientY,lastX:touch.clientX,lastY:touch.clientY,point:p,moved:false,localPan:aimMode||viewScale>1,viewStartPanX:viewPanX,viewStartPanY:viewPanY};event.preventDefault()},{passive:false});
...
video.addEventListener('pointerdown',function(event){if(touchEventsAvailable&&event.pointerType==='touch')return;if(stopped||!critical||critical.readyState!=='open')return;const p=mapPoint(event);if(!p)return;video.setPointerCapture?.(event.pointerId);gesture={id:event.pointerId,pointerType:event.pointerType,startX:event.clientX,startY:event.clientY,lastX:event.clientX,lastY:event.clientY,point:p,moved:false,localPan:aimMode||viewScale>1,viewStartPanX:viewPanX,viewStartPanY:viewPanY};event.preventDefault()});
video.addEventListener('pointermove',function(event){if(!gesture||gesture.pointerType==='touch'||gesture.id!==event.pointerId)return;const dx=event.clientX-gesture.lastX,dy=event.clientY-gesture.lastY;gesture.lastX=event.clientX;gesture.lastY=event.clientY;if(Math.hypot(event.clientX-gesture.startX,event.clientY-gesture.startY)>8)gesture.moved=true;if(gesture.moved){if(gesture.localPan)applyViewTransform(viewScale,gesture.viewStartPanX+event.clientX-gesture.startX,gesture.viewStartPanY+event.clientY-gesture.startY);else sendRealtime({kind:'scroll',deltaX:browserWebRtcScrollDelta(dx),deltaY:browserWebRtcScrollDelta(dy)})}event.preventDefault()});
video.addEventListener('pointerup',function(event){if(!gesture||gesture.pointerType==='touch'||gesture.id!==event.pointerId)return;const g=gesture;gesture=null;if(!g.moved&&Math.hypot(event.clientX-g.startX,event.clientY-g.startY)>8)g.moved=true;if(g.moved){if(!g.localPan){const finalDx=event.clientX-g.lastX,finalDy=event.clientY-g.lastY;if(finalDx||finalDy)sendRealtime({kind:'scroll',deltaX:browserWebRtcScrollDelta(finalDx),deltaY:browserWebRtcScrollDelta(finalDy)})}}else{if(aimMode){event.preventDefault();return}if(keyboardMode)focusKeyboard();const sentAt=performance.now();if(sendPrimaryTap(g.point))lastTapSentAt=sentAt}event.preventDefault()});
video.addEventListener('pointercancel',function(){if(gesture&&gesture.pointerType!=='touch')gesture=null});
keyboard.addEventListener('compositionstart',function(){consumeScheduledKeyboardKey();compositionPhase=browserImeNextCompositionPhase(compositionPhase,'composition_start');suppressKeyboardInput=true});keyboard.addEventListener('compositionend',function(event){consumeScheduledKeyboardKey();settleKeyboardComposition(typeof event.data==='string'?event.data:'')});keyboard.addEventListener('keydown',function(event){if(browserImeKeyboardEventIsCompositionControlled(compositionPhase,event.isComposing,Number(event.keyCode)||0))return;if(event.key==='Backspace'||event.key==='Enter')scheduleKeyboardKey(event.key)});keyboard.addEventListener('beforeinput',function(event){if(browserImeInputEventIsCompositionControlled(compositionPhase,event.isComposing))return;if(event.inputType==='deleteContentBackward'&&inputPolicy.key){consumeScheduledKeyboardKey();if(sendCritical({kind:'key',key:'Backspace'})){if(event.cancelable)event.preventDefault();keyboard.value='';suppressTrailingKeyboardInput()}return}if((event.inputType==='insertLineBreak'||event.inputType==='insertParagraph')&&inputPolicy.key){consumeScheduledKeyboardKey();if(sendCritical({kind:'key',key:'Enter'})){if(event.cancelable)event.preventDefault();keyboard.value='';suppressTrailingKeyboardInput()}return}if((event.inputType==='insertText'||event.inputType==='insertReplacementText'||event.inputType==='insertFromPaste')&&inputPolicy.text&&typeof event.data==='string'&&event.data){if(commitKeyboardText(event.data)&&event.cancelable)event.preventDefault()}});keyboard.addEventListener('input',function(event){if(compositionPhase!=='idle'||event.isComposing)return;if(suppressKeyboardInput){keyboard.value='';return}const value=keyboard.value;if(!value)return;const lineBreak=inputPolicy.key&&(value.endsWith('\\n')||value.endsWith('\\r'));const text=lineBreak?value.slice(0,-1):value;if(text)commitKeyboardText(text);if(lineBreak)sendCritical({kind:'key',key:'Enter'});keyboard.value=''});
...
async function completeHuman(){if(completionState!=='idle'||!completionCapability)return;const completion=completionCapability;completionCapability='';if(doneButton.dataset)doneButton.dataset.completion='';completionState='closing';stopped=true;doneButton.disabled=true;doneButton.setAttribute('aria-disabled','true');doneButton.textContent='Closing…';completionStatus('Closing…');keyboard.blur();closePeer();cap='';reconnectHandle='';clientGeneration=0;try{const response=await fetch('/takeover/api/complete/'+encodeURIComponent(sessionId),{method:'POST',cache:'no-store',keepalive:true,headers:{'x-mcp-takeover-completion':completion}});if(!response.ok)throw new Error('completion unavailable');completionState='closed';doneButton.textContent='Closed';completionStatus('Remote control closed. Return to the requesting workflow.')}catch{completionState='failed';doneButton.textContent='Closed';completionStatus('Completion unavailable. Remote control remains closed.')}}
...
resetKeyboardBuffer();resetViewTransform();armKeyboardFallback();void connect('claim').catch(function(){closePeer();if(relayState==='unavailable')status('Secure relay unavailable');else status('Session unavailable or connection failed');stopped=true});
})();`;
}
function verifyingWebRtcPageHtml(nonce) {
    return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>Human takeover verifying</title><style nonce="${nonce}">:root{font-family:system-ui,-apple-system,sans-serif;color-scheme:dark}html,body{margin:0;min-height:100%;background:#000;color:#fff}main{min-height:100vh;display:grid;place-items:center;padding:24px;text-align:center}p{max-width:34rem;line-height:1.5}</style></head><body><main><p id="status">Verifying… Remote input is disabled.</p></main><script nonce="${nonce}">(()=>{const parts=location.pathname.split('/').filter(Boolean);const id=parts[parts.length-1]||'';const status=document.querySelector('#status');let stopped=false;async function poll(){if(stopped)return;try{const response=await fetch('/takeover/api/webrtc-state/'+encodeURIComponent(id),{cache:'no-store'});if(response.ok){const body=await response.json();if(body&&body.state==='closed'){status.textContent='Remote control closed. Return to the requesting workflow.';stopped=true;return}if(body&&body.state==='verifying'){setTimeout(poll,500);return}}}catch{}status.textContent='Verification unavailable. Remote control remains closed.';stopped=true}void poll()})();</script></body></html>`;
}
...
<textarea id="keyboard" class="keyboard" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" aria-label="Remote keyboard" maxlength="512"></textarea>
</main>
...
export class TakeoverBroker {
    ...
    webRtcSessionInterventions = new Map();
    webRtcTerminalVerificationSessions = new Set();
    verifyingWebRtcSessions = new Set();
    verifiedCompletedWebRtcSessions = new Set();
    webSocketOnlySessions = new Map();
    webSocketRevokeHandlers = new Map();
    ...
    constructor(browser, config, nativeRuntime, webRtcRuntime, hooks = {}) {
        ...
        registerExperimentalWebSocketBrokerPort(this, {
            ...
            createSession: (intervention, principalBinding) => this.createExperimentalWebSocketSession(intervention, principalBinding),
            attachRevokeHandler: (sessionId, handler) => this.attachExperimentalWebSocketRevokeHandler(sessionId, handler),
    ...
    createLink(intervention, principalBinding) {
        ...
        if (this.webSocketOnlySessions.has(locator.id))
            return undefined;
    ...
    createNativeLink(intervention, principalBinding, target) {
        ...
        if (target && (!Number.isSafeInteger(target.processId) || target.processId < 1
            ...
            return undefined;
        const locator = this.sessions.ensure(intervention.id, intervention.epoch, principalBinding);
        if (this.webRtcOnlySessions.has(locator.id) || this.webSocketOnlySessions.has(locator.id))
            return undefined;
    ...
    createWebRtcLink(intervention, principalBinding, target, inputPolicy, options) {
        ...
        if (!normalizedInputPolicy)
            return undefined;
        if (options?.terminalTargetBehavior !== undefined && options.terminalTargetBehavior !== "verifying")
            return undefined;
        const terminalVerification = options?.terminalTargetBehavior === "verifying";
        const locator = this.sessions.ensure(intervention.id, intervention.epoch, principalBinding);
        if (this.nativeOnlySessions.has(locator.id) || this.webSocketOnlySessions.has(locator.id))
            return undefined;
        ...
        if (existingPolicy && !sameWebRtcInputPolicy(existingPolicy, normalizedInputPolicy))
            return undefined;
        if (this.verifyingWebRtcSessions.has(locator.id))
            return undefined;
        if (this.webRtcOnlySessions.has(locator.id)
            && this.webRtcTerminalVerificationSessions.has(locator.id) !== terminalVerification)
            return undefined;
        ...
        if (terminalVerification)
            this.webRtcTerminalVerificationSessions.add(locator.id);
        else
            this.webRtcTerminalVerificationSessions.delete(locator.id);
        if (target) {
        ...
        const completionExpiryCleanup = setTimeout(() => {
            ...
            this.webRtcTerminalVerificationSessions.delete(locator.id);
            this.verifyingWebRtcSessions.delete(locator.id);
    ...
    async completeWebRtcAfterVerification(intervention) {
        ...
        for (const item of completed) {
            if (this.webRtcSessionInterventions.get(item.id) !== intervention.id)
                continue;
            this.verifyingWebRtcSessions.delete(item.id);
     ...
     */
    async completeWebSocketAfterVerification(intervention) {
        const completed = this.sessions.completeAfterVerification(intervention.id, intervention.epoch);
        let matched = false;
        for (const item of completed) {
            if (this.webSocketOnlySessions.get(item.id) !== intervention.id)
                continue;
            matched = true;
    ...
    async handle(request, boundPrincipal) {
        ...
        if (pageMatch) {
            if (this.webSocketOnlySessions.has(pageMatch[1]))
            ...
            try {
                ...
                if (this.verifyingWebRtcSessions.has(pageMatch[1])) {
                    this.sessions.validateLocator(pageMatch[1], boundPrincipal);
                    const nonce = randomBytes(18).toString("base64url");
                    const headers = new Headers(privateHeaders("text/html; charset=utf-8"));
                    headers.set("content-security-policy", `default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'self'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`);
                    return new Response(request.method === "HEAD" ? null : verifyingWebRtcPageHtml(nonce), {
                        status: 200,
                        headers
                    });
                }
        ...
        const apiMatch = /^\/takeover\/api\/(bootstrap|claim|reconnect|webrtc-prepare-claim|webrtc-prepare-reconnect|webrtc-connect|webrtc-diagnostics|webrtc-metrics|webrtc-suspend|webrtc-state|frame|input|done|cancel|complete)\/([A-Za-z0-9-]{8,100})$/.exec(url.pathname);
        if (!apiMatch)
            return json(404, { error: "not_found" });
        const operation = apiMatch[1];
        const id = apiMatch[2];
        if (this.webSocketOnlySessions.has(id))
            return json(404, { error: "takeover_unavailable" });
        if (operation === "webrtc-state") {
            if (request.method !== "GET")
                return json(405, { error: "method_not_allowed" });
            try {
                if (this.verifiedCompletedWebRtcSessions.has(id) && this.sessions.isCompleted(id, boundPrincipal)) {
                    return json(200, { state: "closed" });
                }
                if (this.verifyingWebRtcSessions.has(id)) {
                    this.sessions.validateLocator(id, boundPrincipal);
                    return json(200, { state: "verifying" });
                }
            }
            catch (error) {
                if (error instanceof TakeoverSessionError)
        ...
        if (operation === "complete") {
            ...
            if (this.verifyingWebRtcSessions.has(id)) {
                ...
                return json(409, { error: "takeover_verifying", verifying: true });
            }
            ...
            try {
                ...
                if (error instanceof TakeoverSessionError) {
                    ...
                    if (error.code === "TAKEOVER_CLIENT_ACTIVE") {
                        if (operation === "webrtc-prepare-reconnect") {
                            this.webRtcRuntime.recordDiagnostic({
                                stage: "broker.reconnect.conflict.active_lease",
                                durationMs: Date.now() - prepareStartedAt
                            });
                        }
                        return json(409, { error: "takeover_client_active", retryAfterMs: 500 });
                    }
                    return json(404, { error: "takeover_unavailable" });
                }
                throw error;
        ...
        if (operation === "webrtc-suspend") {
            ...
            try {
                ...
                this.webRtcRuntime.recordDiagnostic({ stage: "broker.generation.release.suspend" });
            }
            catch (error) {
                if (error instanceof TakeoverSessionError)
                    return json(404, { error: "takeover_unavailable" });
        ...
        if (operation === "bootstrap") {
            if (this.nativeOnlySessions.has(id) || this.webRtcOnlySessions.has(id) || this.webSocketOnlySessions.has(id))
                return json(404, { error: "takeover_unavailable" });
            if (request.method !== "GET")
                return json(405, { error: "method_not_allowed" });
        ...
        if (operation === "claim" || operation === "reconnect") {
            if (this.webRtcOnlySessions.has(id) || this.webSocketOnlySessions.has(id))
                return json(404, { error: "takeover_unavailable" });
            if (request.method !== "POST")
                return json(405, { error: "method_not_allowed" });
    ...
    async finalizeCompletion(sessionId, completion, wasNative, wasWebRtc) {
        ...
        this.webRtcTargetWindowIds.delete(sessionId);
        this.webRtcInputPolicies.delete(sessionId);
        this.webRtcTerminalVerificationSessions.delete(sessionId);
        this.verifyingWebRtcSessions.delete(sessionId);
    ...
    createExperimentalWebSocketSession(intervention, principalBinding) {
        if (!this.config.enabled || !this.config.publicBaseUrl || !principalBinding)
            return undefined;
        const locator = this.sessions.ensure(intervention.id, intervention.epoch, principalBinding);
        if (this.nativeOnlySessions.has(locator.id) || this.webRtcOnlySessions.has(locator.id))
            return undefined;
    ...
    attachExperimentalWebSocketRevokeHandler(sessionId, handler) {
        if (!this.webSocketOnlySessions.has(sessionId) || this.webSocketRevokeHandlers.has(sessionId))
            return false;
        this.webSocketRevokeHandlers.set(sessionId, handler);
        return true;
    }
    async completeExperimentalWebSocketSession(completion) {
        ...
        if (this.completionDelivered.has(completion.id))
            return;
        await this.hooks.completed?.({
            interventionId: completion.interventionId,
            epoch: completion.epoch
    ...
    revokeExperimentalWebSocketSession(sessionId) {
        if (!this.webSocketOnlySessions.has(sessionId))
            return;
        this.sessions.revoke(sessionId);
        this.forgetWebSocketOnlySession(sessionId, true);
    }
    forgetWebSocketOnlySession(sessionId, notifyTransport) {
        this.webSocketOnlySessions.delete(sessionId);
        this.completionDelivered.delete(sessionId);
        const handler = this.webSocketRevokeHandlers.get(sessionId);
        this.webSocketRevokeHandlers.delete(sessionId);
        if (notifyTransport && handler)
            void Promise.resolve(handler()).catch(() => undefined);
    }
    forgetWebSocketOnlyIntervention(interventionId, notifyTransport) {
        for (const [sessionId, currentIntervention] of this.webSocketOnlySessions) {
            if (currentIntervention === interventionId)
                this.forgetWebSocketOnlySession(sessionId, notifyTransport);
        }
    }
    webRtcHooks(binding) {
        return {
            ...
            terminal: (cause) => {
                if (!this.webRtcTerminalVerificationSessions.has(binding.takeoverSessionId)
                    || !webRtcTerminalCauseRequiresVerification(cause))
                    return false;
                this.verifyingWebRtcSessions.add(binding.takeoverSessionId);
                this.webRtcTargetProcessIds.delete(binding.takeoverSessionId);
                this.webRtcTargetWindowIds.delete(binding.takeoverSessionId);
                this.webRtcInputPolicies.delete(binding.takeoverSessionId);
                try {
            ...
            disconnected: () => {
                try {
                    this.sessions.releaseClientGeneration(binding.takeoverSessionId, binding.principalBinding, binding.clientBinding, binding.clientGeneration);
                    this.webRtcRuntime?.recordDiagnostic({ stage: "broker.generation.release.peer_loss" });
                }
                catch {
                    // A newer/revoked generation already fences this transport.
                }
    ...
    forgetVerifiedCompletedWebRtcIntervention(interventionId) {
...
function webRtcTerminalCauseRequiresVerification(cause) {
    return cause === "target_missing"
        || cause === "host_protocol"
        || cause === "host_exit"
        || cause === "host_error"
        || cause === "video_drain";
