...
- first-class Browser / Window / Terminal adapterはstable v1 `operatorDiagnosticsSnapshot()` も公開する。cross-surface shapeはidentifier-free / boundedで、WebRTC / Terminal固有factは明示namespaceへ残す。既存の詳細WebRTC diagnostics / latencyとTerminal status APIはtarget固有troubleshooting用として維持する。
...
詳細は [アーキテクチャ](docs/architecture.ja.md)、[Recovery / Observability](docs/recovery-observability.ja.md)、[Product Readiness](docs/product-readiness.ja.md)、[位置づけ](docs/positioning.ja.md)、[ロードマップ](ROADMAP.ja.md)、[リリース手順](RELEASING.ja.md)、[セキュリティポリシー](SECURITY.ja.md)、[Changelog](CHANGELOG.md) を参照してください。
