...
**Status:** 再利用可能なupstreamとして検証済みです。`v0.3.0` が現在のGitHub/source release baselineで、完了済みRecovery / Observability contractとv0.2.0後のbounded hardeningを含みます。`v0.1.0` は最初のsource releaseです。npm packageは引き続き `private: true` で、npmには公開していません。
...
- first-class Browser / Window / Terminal adapterはstable v1 `operatorDiagnosticsSnapshot()` も公開する。cross-surface shapeはidentifier-free / boundedで、WebRTC / Terminal固有factは明示namespaceへ残す。既存の詳細WebRTC diagnostics / latencyとTerminal status APIはtarget固有troubleshooting用として維持する。Managed Browser / Window fallbackはgeneric v1をwidenせずcontent-freeなproduction acceptance evidenceを取得できる、別のstrict `managedOperatorDiagnosticsSnapshot()` contractも公開する。
...
詳細は [アーキテクチャ](docs/architecture.ja.md)、[Product Readiness](docs/product-readiness.ja.md)、[Recovery / Observability](docs/recovery-observability.ja.md)、[位置づけ](docs/positioning.ja.md)、[ロードマップ](ROADMAP.ja.md)、[リリース手順](RELEASING.ja.md)、[セキュリティポリシー](SECURITY.ja.md)、[Changelog](CHANGELOG.md) を参照してください。
...
Linux hostはtarget PIDに属するX11 windowを厳密に1つだけ解決し、そのwindowだけをbounded CPU H.264 pipelineでcaptureします。window discovery / PID ownership / geometry / activation / focusはNode側のfail-closed policyとして維持します。primary pointerのmotion / down / upだけは、小型standalone Xlib/libXtst helperへ委譲し、1 Handoff hostの間は1本のX11 connectionを維持して、各XTEST mutationを `XSync` 完了後にackします。scroll / key / textは既存のbounded OS/window pathを維持します。editable-region / focus parityは別のread-only AT-SPI helperで実装し、process ancestry、accessibility state / role、component extentsだけを読みます。tree走査数とregion数をboundedにし、AT-SPI top-level geometryがexact X11 targetと小さいbounded tolerance内で一致する場合だけ採用します。accessible name / description / text / value / credentialやDOM / CDPは取得しません。Cloud/headless-likeなChromium deploymentではtarget起動時にnative accessibility bridgeを明示有効化する必要があり（例: `ACCESSIBILITY_ENABLED=1` + boundedな `--force-renderer-accessibility=form-controls`）、bridge/treeが unavailable / ambiguousならeditable regionは空、focusはnon-editableとしてfail closedにします。WebRTC generation / TURN / revoke machineryはそのまま再利用し、Browser/profile persistenceはconsumer/deployment側の責務です。
...
このrepositoryをExecution Handoffのupstream source of truthとして扱います。`v0.3.0` が現在の **GitHub/source release only** baselineです。first-class Browser / bounded OS Window / bounded Terminal-PTYを維持しつつ、provider-neutral bounded checkpoint storage、privacy-bounded audit / operator diagnostics v1、deterministic crash/restart conformance、v0.2.0後のbounded Window / Linux / media hardeningを含みます。npm publishは別判断のままで、`private: true` を維持しています。詳細は [リリース手順](RELEASING.ja.md) を参照してください。
