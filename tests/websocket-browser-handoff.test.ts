...
test("Generic Browser WSS serves a principal-bound Handoff-owned browser page without target identity", async () => {
  ...
  assert.doesNotMatch(html, /keyboardMirror|syncKeyboardValue/);
  assert.match(html, /let suppressKeyboardInput=false/);
  assert.match(html, /function commitKeyboardText\(text\)/);
  assert.match(html, /function suppressTrailingKeyboardInput\(\)/);
  assert.match(html, /function settleKeyboardComposition\(data\)/);
  assert.match(html, /compositionend.*settleKeyboardComposition/);
  assert.match(html, /compositionPhase='idle'/);
  assert.match(html, /keyboardInputReleaseTimer=setTimeout/);
  ...
  assert.match(html, /insertReplacementText/);
  assert.match(html, /commitKeyboardText\(event\.data\)/);
  assert.doesNotMatch(html, /compositionend',event=>.*send\(\{kind:'text'/);
