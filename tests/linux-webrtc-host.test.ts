...
test("Linux host keeps Human text off argv and binds capture/input to one target window", () => {
  ...
  assert.match(host, /"mousemove", "--window", String\(this\.geometry\.windowId\)[\s\S]*"getmouselocation", "--shell"/);
  ...
  assert.match(host, /parseMouseLocation/);
  assert.match(host, /pointer did not reach the admitted target point/);
  ...
  assert.match(host, /pointer moved during primary press/);
