...
test("Linux host keeps Human text off argv and binds capture/input to one target window", () => {
  ...
  assert.match(host, /Script mode executes complete lines in order on one xdo\/X11 connection/);
  assert.match(host, /this\.child\.stdin\.write\(`\$\{args\.join\(" "\)\}\\n`/);
  assert.match(host, /session\.command\(\["mouseup", "1"\]\)/);
