...
**Status:** validated reusable upstream. `v0.4.1` is the current GitHub/source release baseline; it retains the v0.4.0 bounded WSS/component-maturity line and adds the internal Desktop Session / Display Backend boundary around the existing Physical Window path. The boundary separates persistent session/display continuity from viewer/transport generations without adding Desktop authority, a public package subpath, virtual/remote backend implementation, or changing Browser/Terminal semantics. `v0.1.0` was the first source release. The npm-package flag remains `private: true`; this project is not published to npm.
...
- Restart recovery is always `reissue_and_revalidate`; it never restores stale execution authority or silently replays an action. Generic audit uses the stable privacy-bounded v1 `ExecutionAuditEvent` union; audit sink failures are observe-only and cannot change authority/checkpoint/recovery semantics.
- First-class Browser, Window, and Terminal adapters also expose stable v1 `operatorDiagnosticsSnapshot()` summaries. The cross-surface shape is identifier-free and bounded; WebRTC and Terminal-specific facts remain in explicit namespaces, while the existing detailed WebRTC diagnostics/latency and Terminal status APIs remain available for target-specific troubleshooting. Managed Browser/Window fallback additionally exposes the separate strict `managedOperatorDiagnosticsSnapshot()` contract for content-free production acceptance evidence without widening generic v1.
...
See [Architecture](docs/architecture.md), [Product readiness](docs/product-readiness.md), [Recovery & observability](docs/recovery-observability.md), [Positioning](docs/positioning.md), [Roadmap](ROADMAP.md), [Release process](RELEASING.md), [Security Policy](SECURITY.md), and [Changelog](CHANGELOG.md).
...
This repository is the upstream source of truth. `v0.4.1` is the current **GitHub/source release only** baseline. It retains the first-class Browser, bounded OS Window, and bounded Terminal/PTY components plus the v0.4.0 bounded WSS/component-maturity work, and adds the internal Desktop Session / Display Backend boundary around the existing Physical Window path. Persistent application/session state and physical display continuity are separated from Human viewer/transport generations; viewer reconnect does not imply application/session teardown, viewer scaling is not physical display resize, stale generations and retargeting fail closed, and the physical backend still rejects dynamic resize. No Desktop authority, virtual/remote backend, public package subpath, or Browser/Terminal semantic change is introduced. For source-release JavaScript consumption, the committed `dist/` tree is consumer-ready: CI stages tracked package metadata + `dist/` without `src/`, installs production dependencies only, and smoke-imports the public entry points/consumer-required exports. npm publication is a separate decision and has not been performed; `private: true` remains in effect. Platform-native helper delivery remains documented separately from this JavaScript artifact contract. See [Release process](RELEASING.md).
