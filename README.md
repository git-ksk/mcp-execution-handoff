...
**Status:** validated reusable upstream. `v0.4.5` is the current GitHub/source release baseline; it preserves the v0.4.4 Target Surface/transport boundary while hardening external Human-surface revoke confirmation, WSS terminal input fencing, and bounded inbound Human-input queues. The patch adds no Target Surface, Desktop authority, OS-support, transport-provider, or Browser/Terminal semantic expansion. `v0.1.0` was the first source release. The npm-package flag remains `private: true`; this project is not published to npm.
...
This repository is the upstream source of truth. `v0.4.5` is the current **GitHub/source release only** baseline. It preserves the first-class Browser, bounded OS Window, and bounded Terminal/PTY components plus the v0.4.4 transport/product boundary while tightening authority correctness: external Human-surface expiry is not treated as provider-side revoke proof, WSS disconnect/revoke intent fences queued Human input immediately, and retained inbound WSS Human-input work is bounded by message count and aggregate bytes with fail-closed overflow. No new Target Surface, Desktop authority, OS-support, transport-provider, virtual/remote backend, public package subpath, or Browser/Terminal semantic claim is introduced. For source-release JavaScript consumption, the committed `dist/` tree is consumer-ready: CI stages tracked package metadata + `dist/` without `src/`, installs production dependencies only, and smoke-imports the public entry points/consumer-required exports. npm publication is a separate decision and has not been performed; `private: true` remains in effect. Platform-native helper delivery remains documented separately from this JavaScript artifact contract. See [Release process](RELEASING.md).
