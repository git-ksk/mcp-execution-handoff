...
**Status:** validated reusable upstream. `v0.4.2` is the current GitHub/source release baseline; it retains the v0.4.1 Desktop Session / Display Backend boundary and fixes credential-safe external Human-surface expiry so stale cached locators are never reused as active. The patch adds no Target Surface, Desktop authority, OS-support, transport-provider, or Browser/Terminal semantic expansion. `v0.1.0` was the first source release. The npm-package flag remains `private: true`; this project is not published to npm.
...
This repository is the upstream source of truth. `v0.4.2` is the current **GitHub/source release only** baseline. It retains the first-class Browser, bounded OS Window, and bounded Terminal/PTY components plus the v0.4.1 Desktop Session / Display Backend boundary, and fixes credential-safe external Human-surface expiry: expired cached locators are not returned as active, matching retries fail explicitly and require a separate fresh `begin()`, and cleanup never restores Human/Agent authority or replays Human input. No new Target Surface, Desktop authority, OS-support, transport-provider, virtual/remote backend, public package subpath, or Browser/Terminal semantic claim is introduced. For source-release JavaScript consumption, the committed `dist/` tree is consumer-ready: CI stages tracked package metadata + `dist/` without `src/`, installs production dependencies only, and smoke-imports the public entry points/consumer-required exports. npm publication is a separate decision and has not been performed; `private: true` remains in effect. Platform-native helper delivery remains documented separately from this JavaScript artifact contract. See [Release process](RELEASING.md).
