...
- First-class Browser, Window, and Terminal adapters also expose stable v1 `operatorDiagnosticsSnapshot()` summaries. The cross-surface shape is identifier-free and bounded; WebRTC and Terminal-specific facts remain in explicit namespaces, while the existing detailed WebRTC diagnostics/latency and Terminal status APIs remain available for target-specific troubleshooting.
