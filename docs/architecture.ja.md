...
mobile Safariのsoftware keyboard activationは明示的なHuman gestureのままです。editable-region metadataは意図したfieldをlegibleにする補助には使えても、DOM/value inspectionやsynthetic focusのauthorityにはせず、streamed remote fieldからuser gestureなしにiOS keyboardをprogrammaticに開けるとはclaimしません。そのため明示 `⌨︎` controlをfirst-class fallback/baselineとして維持し、一度enableした後はremote tapでもkeyboard modeを維持し、IME replacement / Enter / Backspace semanticsをtransport横断で統一します。IME/text commit sequencingもtransport-neutralにし、browser clientは `idle -> composing -> settling` を遷移します。composition中のpreeditと確定用keyはlocalに保持し、通常文字は確定済み `beforeinput.data`、IMEはcomposition確定結果だけを1回remote commitします。Safariがその直後に発火するtrailing `input` はone-shotでconsumeし、hidden textareaは各commit後に空へ戻すため、累積mirror差分・preedit replay・stale buffer再送を行いません。Human textをdiagnosticsやdurable stateへ追加しません。
