...
For local power-loss hardening, the signed-file provider writes a new `0600` temporary file, flushes that file with `fsync`, atomically renames it into place, and then flushes the parent directory on platforms/filesystems where Node exposes a supported directory `fsync`. A successful removal also flushes that parent directory so `clear()` does not claim durable deletion before the directory entry is persisted. Any ordinary file/directory barrier failure is a synchronous `write()` failure; an active Human intervention is therefore cancelled/fenced by `ExecutionHandoffRuntime` before the error escapes. Explicitly unsupported directory barriers (`EINVAL` / `ENOTSUP` / `EOPNOTSUPP`) do not make the provider unusable, but weaken only the crash-durability claim for the rename. Windows always performs the file `fsync` but does not claim a portable directory-handle `fsync` guarantee. Pre-provisioning the private `0700` checkpoint directory gives the strongest local-filesystem boundary; recursively creating a previously absent directory is not claimed to durably flush every ancestor directory entry. No filesystem can provide a universal sudden-power-loss guarantee beyond its own documented `fsync`/rename semantics.
...
For physical acceptance, capture this same snapshot at four boundaries: before takeover, after managed fallback, immediately after failure, and after completion. The failure snapshot is the decision point for recovery work: do not infer an input-recovery fix from `Session unavailable` alone. Consumers that need automatic production export may supply `onManagedOperatorDiagnosticEvent`; it receives only the same bounded `{ kind }` event records. The callback is observe-only and exceptions are contained, so it cannot grant, revoke, restore, or widen authority. A consumer may use the callback as a trigger to read `managedOperatorDiagnosticsSnapshot()` and export that strict snapshot to its operator log/telemetry path.
