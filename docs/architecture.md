...
A consumer first enters the normal handoff lifecycle and gives exclusive authority to the Human. Only then may `CredentialSafeHumanSurfaceRuntime.begin()` create an external operator session. The runtime binds that session to the active intervention id, resource epoch, and principal binding. Only one session may be active, and a duplicate begin is idempotent only for the same binding **while the cached surface is still unexpired**.

Provider output is deliberately narrowed to bounded fields: provider kind, intervention id, epoch, principal binding, session id, operator locator, and optional expiry. `expiresAt`, when present, is an absolute Unix timestamp in milliseconds and is an authority/liveness cutoff for the cached external surface. Extra provider data is not retained. In particular, credentials, browser cookies, tokens, screenshots, DOM/network data, and provider-specific opaque metadata must not be used as continuity material.

An expired cached surface is never returned by `getActive()` or an idempotent `begin()`. A matching `begin()` that discovers cached expiry clears that stale cache, attempts provider revoke only as best-effort cleanup, and returns the explicit `EXTERNAL_SURFACE_EXPIRED` error. It does **not** create a replacement session in the same call. The consumer must discard the stale locator and, only if the same Human intervention should continue, call `begin()` again explicitly to request a fresh provider session. An already-expired provider grant is rejected and best-effort revoked before it can become active. None of these expiry paths complete the Human intervention, restore Agent authority, replay Human input, or attest target-service authentication.

Before restoring automation, the consumer must revoke the external session or establish that its declared expiry has already made the cached surface inactive, then verify any consumer-specific execution boundary, such as closing a normal browser and releasing its dedicated profile lock. Human completion then advances through the existing `verifying -> ready_to_resume` lifecycle. Authentication success must be revalidated from fresh browser state, and stale pre-auth semantic actions must not be replayed.
...
On mobile Safari, software-keyboard activation remains an explicit Human gesture. Editable-region metadata may make the intended field legible but never authorizes DOM/value inspection or synthetic focus, and Handoff does not claim that a streamed remote field can programmatically open the iOS keyboard without a user gesture. The explicit `⌨︎` control is therefore the first-class fallback/baseline; once enabled, remote taps keep keyboard mode active and IME replacement, Enter, and Backspace stay transport-normalized. IME/text commit sequencing is transport-neutral and Safari-owned: the hidden textarea remains intact for the lifetime of the keyboard session, composition preedit updates (`insertCompositionText` / `deleteCompositionText`) remain local, and only a finalized `insertFromComposition` or an ordinary non-composing `input` advances the committed mirror and emits its DOM delta. The client does not clear the textarea between edits, so WebKit can preserve its own IME state; failed sends are not replayed later. The v0.4.3 physical baseline covers the iOS system keyboard, while third-party keyboards with different replacement streams are tracked separately. Enter/Backspace keydown is retained only as a 250 ms pending-key fallback and is cancelled when the corresponding DOM input mutation arrives, avoiding duplicate newline/delete delivery on iOS Safari. Human text is never added to diagnostics or durable state.
...
On macOS Window WSS reconnect, each fresh client generation receives the latest exact-window frame retained by the authority-bound helper as its first frame, while any pending next-frame capture owned by the prior generation is cancelled. This restores a static view without waiting for content mutation and never revives stale frame/input authority.

Third-party iOS keyboards may represent candidate replacement as ordinary DOM `insertText` / `deleteContentBackward` instead of a composition lifecycle. Browser Human Input therefore keeps suffix-replacement semantics transport-neutral: the hidden textarea DOM remains authoritative by default, while an ordinary non-composing `insertText` may fill only a missing trailing portion when its bounded `InputEvent.data` is a strict extension of the DOM insertion. The hidden mirror is normalized to that completed value before later edits. `insertFromComposition` keeps the existing system-keyboard DOM-authoritative path, and `keyCode=229` alone never proves a commit. This does not add cursor keys, widen input authority, or log Human text.

WSS first-frame startup is measured without content or identity. The browser records initial connect-to-`ready`, `ready`-to-first-`img.onload`, receive-to-decode, and total connect-to-first-frame durations in the browser clock domain. The macOS exact-window surface records helper preparation and next-frame wait in the host clock domain; on a newly started helper, the first already-validated JPEG is reused as the first capture rather than discarded in favor of a second frame. `ready` therefore means transport/authority readiness, not that pixels are already visible: the Human page remains in `Human authority active · preparing view…` until the first valid frame load completes. These metrics are bounded distributions only and do not retain frame bytes, target/process/window identity, principals, URLs, or Human input.
...
Window WSS uses a 50 ms (~20 fps) healthy-path pump by default. Capture remains single-flight, no capture runs without an authenticated active client, and the WebSocket channel still keeps at most one newest pending frame under send/backpressure. Content-free active diagnostics expose bounded sent/dropped frame counts, backpressure observations, and current/maximum buffered byte counts; they contain no frame bytes, target identity, Human input, capability, or principal data. Consumers may still configure a slower 50–2000 ms interval when deployment constraints require it.
