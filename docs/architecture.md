...
Before restoring automation, the consumer must revoke the external Human surface and verify its consumer-specific execution boundary. A local profile-switch owner may need to close a normal browser and verify dedicated-profile lock release. A hosted shared-session owner instead keeps the exact browser session alive while Human authority is exclusive, detaches the automation client, revokes the Live View handoff, and then creates a fresh automation attachment to that same browser session. Browser ownership and provider lifecycle remain consumer responsibilities rather than generic-core concepts.
