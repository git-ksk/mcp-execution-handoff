...
Before restoring automation, the consumer must revoke the external Human surface and verify its consumer-specific execution boundary. A local profile-switch owner may need to close a normal browser and verify dedicated-profile lock release. A same-session Thin Takeover owner may instead keep the browser session alive while Human authority is exclusive: Agent-owned automation input authority is detached/fenced, a separate Human-owned intervention/epoch-bound capture/input attachment may exist, revocation aborts active frame streams, that Human attachment is closed, and only then is a fresh automation attachment created. The invariant is exclusive authority rather than “no CDP/driver connection exists during Human control.” Browser ownership, capture, encoding, and provider lifecycle remain consumer responsibilities rather than generic-core concepts.

Human completion then advances through the existing `verifying -> ready_to_resume` lifecycle. Authentication success must be revalidated from fresh browser state, and stale pre-auth semantic actions must not be replayed. Credential/MFA/passkey/cookie/browser-session bearer material and provider API keys must stay outside MCP/model/logs; a locator exposed northbound must not itself be a secret bearer capability. Passkey/WebAuthn remains Human/provider-controlled and is not bypassed.
...
For low-latency adapters, the broker can carry an optional bounded frame stream over an authenticated streaming HTTP response. This changes only frame delivery: input, principal binding, epoch fencing, client generation, expiry, and revocation keep the same semantics. Revocation aborts active streams for that intervention. The browser adapter owns capture, encoded-vs-raw frame policy, encoding, and backpressure; CDP/WebRTC/provider lifecycle remain outside the generic broker.
